...
export class MarketGroupPanel implements OnDestroy {
  ...
  private initiateComponentState(){
    this.state = {
      ...
      configurator: {
        dto: this.dtoService.createSecurityDefinitionConfigurator(false, false),
  ...
  public onClearConfig(){
    this.state.configurator.dto = this.dtoService.createSecurityDefinitionConfigurator(false, false);
  }
  ...
  private updateConfigurator(selectedGroupList: Array<SecurityGroupDTO>){
    ...
    const newConfig = this.dtoService.createSecurityDefinitionConfigurator(false, false);
